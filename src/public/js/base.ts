var gameBoard: Tile[][] = null
var boardMaxX: number = NaN
var boardMaxY: number = NaN
...
async function mobileNavToggleOn(): Promise<void> {
...
async function mobileNavToggleOff(): Promise<void> {
    for (var i = 3; i > 0; i--) {
        $(`.mobile-nav ul li:nth-child(${i})`).toggle('slow', 'swing')
        await sleep(70)
    }
...
async function fadeInPageDesktop(): Promise<void> {
 ...
 */
function setDesktopHeaderLookandFeel(): void {
...
function createGrid(gameDiv: JQuery<HTMLElement>, columns: number, rows: number) {
    var blockSize = '10px '
...
class Snake {
    ...
    setCurrent(row: number, col: number) {
        this.head = new BoxNode(gameBoard[row][col])
        this.head.tile.setWhite()
    }
    checkSafeMove(x: number, y :number) {
        if (x < 0 || x >= boardMaxX)
     ...
     * @param {Color} color color to change snake to
     */
    changeSnakeColor(color: string) {
        var n = this.head
        while (n != null) {
            n.tile.setColor(color)
            n = n.next
        }
    }
}
 ...
 * @param {number} dir 
 */
function doMove(snake: Snake, dir: number) {
    switch (dir) {
        case 0:
            snake.advanceUp()
            break;
 ...
 * @returns 
 */
function checkMove(snake: Snake, dir: number) {
    switch (dir) {
        case 0:
